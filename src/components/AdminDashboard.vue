...
const emit = defineEmits<{ focusBuilding: [buildingId: string] }>()
  ...
  occupancyChart.setOption({
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        return `<div style="font-size:12px;cursor:pointer">${d.name}<br/>占用 ${d.busy}/${d.total} 间 (${d.rate}%)<br/><span style="color:#999">点击聚焦</span></div>`
      },
      backgroundColor: 'rgba(255,255,255,0.95)', borderColor: '#e5e7eb', textStyle: { color: '#1f2937' },
    },
  })
}
...
<template>
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-3 gap-1.5">
      <div class="p-2.5 rounded-xl bg-[#0e3a67]/5 border border-[#0e3a67]/10 cursor-pointer hover:bg-[#0e3a67]/10 transition" @click="emit('focusBuilding', '')">
        <div class="text-[9px] text-text-secondary">教室占用率</div>
        <div class="text-lg font-bold text-brand-navy mt-0.5">{{ store.classroomOccupancyRate }}<span class="text-xs font-normal">%</span></div>
      </div>
      <div class="p-2.5 rounded-xl bg-[#1e6fdb]/5 border border-[#1e6fdb]/10">
        <div class="text-[9px] text-text-secondary">会议室占用率</div>
        <div class="text-lg font-bold text-brand-blue mt-0.5">{{ store.meetingOccupancyRate }}<span class="text-xs font-normal">%</span></div>
      </div>
      <div class="p-2.5 rounded-xl bg-[#ef4444]/5 border border-[#ef4444]/10">
        <div class="text-[9px] text-text-secondary">待处理工单</div>
        <div class="text-lg font-bold text-status-repair mt-0.5">{{ store.tickets.filter(t => t.status !== 'done').length }}</div>
      </div>
      <div class="p-2.5 rounded-xl bg-[#9c7720]/5 border border-[#9c7720]/10">
        <div class="text-[9px] text-text-secondary">今日总能耗</div>
        <div class="text-lg font-bold text-brand-gold mt-0.5">{{ store.totalEnergy }}<span class="text-[10px] font-normal"> kWh</span></div>
      </div>
      <div class="p-2.5 rounded-xl bg-[#10b981]/5 border border-[#10b981]/10">
        <div class="text-[9px] text-text-secondary">当前在校人流</div>
        <div class="text-lg font-bold text-status-free mt-0.5">{{ store.totalTraffic }}<span class="text-[10px] font-normal"> 人</span></div>
      </div>
      <div class="p-2.5 rounded-xl bg-[#6b7280]/5 border border-[#6b7280]/10">
        <div class="text-[9px] text-text-secondary">全校总占用率</div>
        <div class="text-lg font-bold text-text-primary mt-0.5">{{ store.occupancyRate }}<span class="text-xs font-normal">%</span></div>
      </div>
    </div>

    <!-- 热力层开关 -->
    <div class="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-gray-50 border border-gray-100">
      <span class="text-[10px] text-text-secondary mr-1">3D 热力</span>
      <button
        @click="setHeatmap('off')"
        :class="['px-2 py-0.5 rounded text-[10px] transition', store.heatmapMode === 'off' ? 'bg-brand-navy text-white' : 'bg-gray-100 text-text-secondary hover:bg-gray-200']"
      >关闭</button>
      <button
        @click="setHeatmap('energy')"
        :class="['px-2 py-0.5 rounded text-[10px] transition', store.heatmapMode === 'energy' ? 'bg-brand-gold text-white' : 'bg-gray-100 text-text-secondary hover:bg-gray-200']"
      >能耗</button>
      <button
        @click="setHeatmap('traffic')"
        :class="['px-2 py-0.5 rounded text-[10px] transition', store.heatmapMode === 'traffic' ? 'bg-status-free text-white' : 'bg-gray-100 text-text-secondary hover:bg-gray-200']"
      >人流</button>
    </div>

    <!-- 占用率排行 -->
    <div>
      <div class="text-[11px] text-text-secondary font-medium mb-1.5 flex items-center gap-1.5">
        <span class="w-1 h-1 rounded-full bg-brand-navy"></span>
        各楼占用率排行
        <span class="text-[9px] text-text-secondary/50 ml-auto">点击聚焦</span>
      </div>
